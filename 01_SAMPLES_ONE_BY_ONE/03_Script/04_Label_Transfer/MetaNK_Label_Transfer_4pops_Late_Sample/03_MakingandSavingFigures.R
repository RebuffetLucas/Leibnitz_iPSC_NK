...
p21 = ggplot(PBMC_Blood_All2@meta.data, aes_string(x=QUERY_CLUSTER_COLNAME, fill= "predicted.id")) + geom_bar(position="fill") + theme(text = element_text(size=20),axis.text.x = element_text(angle = 90))  + scale_fill_manual(values= palette2) + ggtitle("Data Only") + theme_minimal() + theme(text = element_text(size = 20))

p21 <- ggplot(PBMC_Blood_All2@meta.data, 
...
pdf(file.path(PATH_ANALYSIS_OUTPUT_FIGURES, "UMAP_Predicted_Id_V2.pdf"),  width = 10, height = 6)
p32
dev.off()
...
if (DO_REMOVE_VERY_LITTLE_IDENT_SUBSET==TRUE){
  ...
  predicted_counts <- table(PBMC_Blood_All2$predicted.id)
  ...
  PBMC_Blood_All2 = subset(PBMC_Blood_All2, subset= predicted.id %in% predicted_less_than_treshold , invert=TRUE)
}else{PBMC_Blood_All2 = PBMC_Blood_All2
}
...
alluvial_data <- meta_data %>%
  dplyr::select(seurat_clusters, predicted.id) %>%
  dplyr::count(seurat_clusters, predicted.id)

#Remove NA if neaded
alluvial_data = na.omit(alluvial_data)

# Create the alluvial plot
p32 = ggplot(alluvial_data, aes(
  axis1 = seurat_clusters,
  axis2 = predicted.id,
  y = n
)) +
...
pdf(file.path(PATH_ANALYSIS_OUTPUT_FIGURES, "AlluvialPlot_Predicted_Id.pdf"),  width = 6, height = 12)
p32
dev.off()
...
for (i in names(Markers_NK_123)){
  PBMC_Blood_All2 = AddModuleScore(PBMC_Blood_All2, features = as.list(Markers_NK_123[[i]]), pool= NULL ,name= i , seed=19)
}
