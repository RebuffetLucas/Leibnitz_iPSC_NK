...
#Extract from a list of Seurat Object
if (DO_EXTRACT_FROM_A_SEURAT_LIST==TRUE){
  PBMC_Blood_All = PBMC_Blood_All[[NAME_SEURAT_OBJECT_TO_EXTRACT]]
}

#Harmonize meta data before merging
...
if(DO_SUBSET_VARIABLE==TRUE){
  PBMC_Blood_subset = SetIdent(PBMC_Blood_All , value=VARIABLE_TO_SUBSET )
  PBMC_Blood_subset = subset(PBMC_Blood_subset, idents = CATEGORIES_TO_SUBSET)
}else{
